...
class bookCRUD(Resource):
    ...
    def put(self, booking_id=0):
        ...
        try:
            ...
            rating = request.json['rating'] 
            payment_amount = request.json['payment_amount']
            cur3.execute("UPDATE booking SET booking_date=%s, booking_time=%s, rating=%s, payment_amount=%s WHERE booking_id=%s", (booking_date, booking_time, rating, payment_amount, booking_id,))
            dbconnection.commit()

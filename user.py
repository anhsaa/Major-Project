...
class userCRUD(Resource):
    ...
    def post(self):
        ...
        owners_data = cur.fetchone()
        if user_data:
            return jsonify({'msg': 'User Logged In', 'status_code': '200'})
        elif owners_data:
            return jsonify({'msg': 'Owner Logged In', 'status_code': '201'})
        else:
...
class registerCRUD(Resource):
    ...
    def post(self):
        ...
        cur1.execute("INSERT INTO user(user_name, password, email, contact) values(%s,%s,%s,%s)",
                     (user_name, password, email, contact,))
        # dat = cur1.fetchone()
        dbconnection.commit()
